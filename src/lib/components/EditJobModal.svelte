<script lang="ts">
    import { updateJob } from '$lib/api/job-service';
    import type { ErrorResponse } from '$lib/api/types/error';
    import type { Job, JobType, UpdateJobRequest } from '$lib/api/types/job';
    ...
    let formData = $state<UpdateJobRequest>({
        companyName: '',
        positionTitle: '',
        salaryGross: 0,
        jobType: 'clt',
    ...
    async function handleSubmit(e: Event) {
        ...
        try {
            await updateJob(profileId, currentJob.id, formData);
            modalElement.close();
            await resetForm();
            onSuccess?.();
        } catch (err) {
    ...
    let monthlyHours = $derived((formData.dailyWorkHours ?? 0) * (formData.workdaysPerMonth ?? 0));
    ...
    let hourlyRate = $derived(monthlyHours > 0 ? (formData.salaryGross ?? 0) / monthlyHours : 0);
</script>
